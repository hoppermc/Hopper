...
data class SpigotEntity(
    ...
    class EntityCacheLoader : CacheLoader<String, EntityResult>() {
        override fun load(key: String): EntityResult {
            val entity = Bukkit.getEntity(UUID.fromString(key))
            return when (entity) {
                null -> EntityResult(false, null)
                else -> EntityResult(true, entity)
            }
        }
    }
}
