...
import dev.helight.hopper.ecs.ComponentGroupExtensions.migrateMultipleDown
import dev.helight.hopper.ecs.ComponentGroupExtensions.migrateTo
...
class ArchetypeStorage {
    ...
    fun backup(): StoreSnapshot {
        ...
        try {
            snapshot = StoreSnapshot(mappedArchetypes.values.map { it.snapshot() }.toList())
        } finally {
            r.unlock()
        }
        return snapshot!!
